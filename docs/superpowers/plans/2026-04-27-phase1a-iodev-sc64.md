...
- [ ] **Step 5: Commit (Makefile + patcher only — NOT the linker script)**

`linker_scripts/us/rev1/starfox64.ld` is gitignored (per `CLAUDE.md`'s "Linker script" section) — it's generated by splat and modified per-checkout by the patcher. We commit the **inputs to the patcher** (`tools/patch_linker_script.py`) and the build-system change (`Makefile`); the script regenerates the right entries on any clone via `make practice`.

```bash
git add Makefile tools/patch_linker_script.py
# Do NOT git add linker_scripts/us/rev1/starfox64.ld — it's gitignored.
git commit -m "build: add lib/ source discovery and iodev linker entries"
```

If `git status` shows `linker_scripts/us/rev1/starfox64.ld` as modified, that's expected — the patcher updated it in-place. Leave it untracked.
