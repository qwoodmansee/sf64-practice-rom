-- Test: Selecting BOSSES -> GORAS from the level select warps into Titania
-- with the Titania Goras boss spawning.

local H = dofile("tests/harness.lua")
H.test_name = "boss_test_goras"

local OBJ_BOSS_TI_GORAS = 306  -- include/sf64object.h
local BOSS_INDEX         = 6   -- sBossList[6]
...
-- Scroll right to GORAS (boss index 6).
...
-- gPracticeForceCarrier must remain false for Goras.
...
-- Verify we landed in Titania.
H.assert_eq(H.read_s32(H.S.gCurrentLevel), H.S.const.LEVEL_TITANIA,
    "current level is TITANIA")

-- Wait for the Goras boss to spawn (timeout 600 frames ~10s).
...
H.assert_eq(boss_id, OBJ_BOSS_TI_GORAS, "boss id is correct")
