-- Test: Selecting BOSSES -> GOLEMECH from the level select warps into Venom 1
-- with the Venom 1 Golemech boss spawning.

local H = dofile("tests/harness.lua")
H.test_name = "boss_test_golemech"

local OBJ_BOSS_VE1_GOLEMECH = 319  -- include/sf64object.h
local BOSS_INDEX             = 8   -- sBossList[8]
...
-- Scroll right to GOLEMECH (boss index 8).
...
-- gPracticeForceCarrier must remain false for Golemech.
...
-- Verify we landed in Venom 1.
H.assert_eq(H.read_s32(H.S.gCurrentLevel), H.S.const.LEVEL_VENOM_1,
    "current level is VENOM 1")

-- Wait for the Golemech boss to spawn (timeout 600 frames ~10s).
...
H.assert_eq(boss_id, OBJ_BOSS_VE1_GOLEMECH, "boss id is correct")
