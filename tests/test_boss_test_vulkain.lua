-- Test: Selecting BOSSES -> VULKAIN from the level select warps into Solar
-- with the Solar Vulkain boss spawning.

local H = dofile("tests/harness.lua")
H.test_name = "boss_test_vulkain"

local OBJ_BOSS_SO_VULKAIN = 315  -- include/sf64object.h
local BOSS_INDEX           = 4   -- sBossList[4]
...
-- Scroll right to VULKAIN (boss index 4).
...
-- gPracticeForceCarrier must remain false for Vulkain.
...
-- Verify we landed in Solar.
H.assert_eq(H.read_s32(H.S.gCurrentLevel), H.S.const.LEVEL_SOLAR,
    "current level is SOLAR")

-- Wait for the Vulkain boss to spawn (timeout 600 frames ~10s).
...
H.assert_eq(boss_id, OBJ_BOSS_SO_VULKAIN, "boss id is correct")
