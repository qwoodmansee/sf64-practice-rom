-- Test: Selecting BOSSES -> GORGON from the level select warps into Area 6
-- with the Area 6 Gorgon boss spawning.

local H = dofile("tests/harness.lua")
H.test_name = "boss_test_gorgon"

local OBJ_BOSS_A6_GORGON = 302  -- include/sf64object.h
local BOSS_INDEX          = 7   -- sBossList[7]
...
-- Scroll right to GORGON (boss index 7).
...
-- gPracticeForceCarrier must remain false for Gorgon.
...
-- Verify we landed in Area 6.
H.assert_eq(H.read_s32(H.S.gCurrentLevel), H.S.const.LEVEL_AREA_6,
    "current level is AREA 6")

-- Wait for the Gorgon boss to spawn (timeout 600 frames ~10s).
...
H.assert_eq(boss_id, OBJ_BOSS_A6_GORGON, "boss id is correct")
