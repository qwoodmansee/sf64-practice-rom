...
void PlayerShot_TorpedoTrail_Setup(EffectTorpedoTrail* effect, f32 xPos, f32 yPos, f32 zPos) {
    ...
    effect->obj.id = OBJ_EFFECT_TORPEDO_TRAIL;
...
void PlayerShot_SpawnTorpedoTrail(f32 xPos, f32 yPos, f32 zPos) {
    ...
    for (i = 0; i < ARRAY_COUNT(gEffects); i++) {
        if (gEffects[i].obj.status == OBJ_FREE) {
            PlayerShot_TorpedoTrail_Setup(&gEffects[i], xPos, yPos, zPos);
            break;
        }
    }
}
...
void PlayerShot_Impact(PlayerShot* shot) {
    ...
    if ((shot->obj.id == PLAYERSHOT_BOMB) ||
        ...
        if (gCurrentLevel == LEVEL_AQUAS) {
            ...
            Effect_FireSmoke1_Spawn3(shot->obj.pos.x, shot->obj.pos.y, shot->obj.pos.z, 4.0f);
        }
    } else {
    ...
    if ((gGroundType != GROUND_4) && (gLevelType == LEVELTYPE_PLANET) && (gGroundSurface <= SURFACE_GRASS) &&
        ...
        for (i = 0; i < 50; i++) {
            if (gEffects[i].obj.status == OBJ_FREE) {
                ...
                Effect_FireSmoke2_Spawn3(xPos, yPos, zPos, 2.0f);
                break;
            }
        }
    }
            ...
            for (i = 0; i < count; i++, hitboxData += 6) {
                ...
                if ((fabsf(hitbox->z.offset + obj->pos.z - shotPz) < (hitbox->z.size + 50.0f)) &&
                    ...
                    if (obj->id < OBJ_SCENERY_MAX) {
                        Effect_TimedSfx_Spawn(&shot->obj.pos, NA_SE_EN_REFLECT);
                    }
                    return i + 1;
                }
            }
...
void PlayerShot_ApplyDamageToActor(PlayerShot* shot, Actor* actor, s32 hitIndex) {
    ...
    if (((actor->obj.id == OBJ_ACTOR_EVENT) && (actor->dmgPart == 0) &&
        ...
        if ((shot->obj.id == PLAYERSHOT_LOCK_ON) || (shot->obj.id == PLAYERSHOT_BOMB)) {
            ...
            actor->timer_0C2 = 3;
            Effect_Sparkle_Spawn(actor->obj.pos.x, actor->obj.pos.y, actor->obj.pos.z, 8.0f);
...
void PlayerShot_CollisionCheck(PlayerShot* shot) {
            ...
            for (i = 0, player = gPlayer; i < gCamCount; i++, player++) {
                ...
                if ((i != shot->sourceId) &&
                    ...
                    if (player->barrelRollAlpha >= 100) {
                        ...
                        Effect_Sparkle_Spawn(player->pos.x, player->pos.y, player->trueZpos, 8.0f);
                    } else {
                        PlayerShot_Impact(shot);
                        if (player->hitTimer == 0) {
                            ...
                            if (player->form != FORM_ON_FOOT) {
                                Effect_FireSmoke3_Spawn(player->pos.x, player->pos.y, player->trueZpos, 2.0f);
                            }
                        }
                    }
                }
    ...
    if ((gGroundType == GROUND_4) &&
        ...
        PlayerShot_Impact(shot);
        if (shot->obj.id != PLAYERSHOT_LOCK_SEARCH) {
            Effect_TiDust_Spawn(shot->obj.pos.x, shot->obj.pos.y, shot->obj.pos.z, 2.0f, 255, 15, 0);
        }
    }
    if (gUseDynaFloor &&
...
void PlayerShot_UpdateBeam(PlayerShot* shot, s32 index) {
    ...
    if ((shot->obj.pos.y < gGroundHeight) && (gGroundType != GROUND_4)) {
        ...
        if (gCurrentLevel == LEVEL_AQUAS) {
            ...
            Effect_Effect364_Spawn(shot->obj.pos.x, shot->obj.pos.y, shot->obj.pos.z, 3.0f);
            Effect_Effect364_Spawn(shot->obj.pos.x, shot->obj.pos.y, shot->obj.pos.z, 3.0f);
            Aquas_Bubble_Spawn(RAND_FLOAT_CENTERED(10.0f) + shot->obj.pos.x,
        ...
        if (effect->obj.status == OBJ_ACTIVE) {
            ...
            if (sqrtf(SQ(dx) + SQ(dy) + SQ(dz)) < radius) {
                ...
                if (effect->info.unk_16 == 2) {
                    ...
                    if ((effect->obj.id == OBJ_EFFECT_SO_FLARE) && (effect->state == 0)) {
                        effect->state = 1;
                        effect->alpha = 176;
                        effect->unk_4C = 0;
    ...
    switch (shot->obj.id) {
        ...
        case PLAYERSHOT_LOCK_ON:
            if (shot->scale > 1.5f) {
                ...
                Object_Kill(&shot->obj, shot->sfxSource);
                Effect_TimedSfx_Spawn(&shot->obj.pos, NA_SE_SPREAD_EXPLOSION);
