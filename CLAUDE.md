...
## CRITICAL: main-segment RAM budget — scene stacks must end below 0x80281000

The level asset window (`ovl_i1_VRAM`) floats after `main`: every byte of
practice code/data added to `main` pushes ALL level asset loads upward. The
`buffers` segment (`gDramStack`, `gOSYieldData`, `gZBuffer`,
`gTaskOutputBuffer`, `gAudioHeap`, framebuffers) is FIXED at `0x80281000`.
When a big level's asset stack crosses that line, its assets are DMA'd over
the Z-buffer and then shredded by RDP depth writes every frame — the level
crashes/wedges ON HARDWARE ONLY (HLE emulators never write the Z-buffer back
to RDRAM, so mupen64plus can NEVER catch this). This caused the 2026-07-11
"Katt freeze" (Zoness), plus Aquas/Macbeth/Andross/options-menu instability:
10 scene setups were overflowing, worst by 0x11800.

- Vanilla headroom for `main` growth is only ~0xA710 (~43 KB) total.
- New practice code belongs in a Pak segment, NOT in `main`.
- There is NO free stock-RAM region for practice code: anything placed in
  `[ovl_i1_VRAM, 0x80281000)` (e.g. the old 0x801F4000 idea) gets DMA'd over
  by big levels.
- `check_scene_stack_fits_buffers` in `tools/practice_invariants.py` enforces
  the budget; never weaken it.
- Full writeup: `docs/superpowers/specs/2026-07-11-scene-window-zbuffer-overlap.md`
...
> **Default home is `.practice_late_pak` (Pak RAM), NOT main.** Since the
> 2026-07-11 scene-window fix, new practice feature code goes in
> `PRACTICE_LATE_PAK_PRACTICE_OBJS` in `tools/patch_linker_script.py` and
> is only reachable behind `Practice_PakReady()` (the gates in
> `practice_main.c` cover code called from Practice_Update/Draw). A file
> may go in `PRACTICE_OBJS` (main) ONLY if engine/sys code must reference
> its symbols unconditionally — and then keep it tiny; the scene-stack
> invariant will fail the build if main grows too far. Engine-called
> functions and engine-written globals belong in `practice_shim.c`.
...
5. Wire Update/Draw calls into `Practice_Update()`/`Practice_Draw()` in
   `practice_main.c` — inside the `Practice_PakReady()` gated regions
...
7. Add filename (no extension) to `PRACTICE_LATE_PAK_PRACTICE_OBJS` in
   `tools/patch_linker_script.py` (or `PRACTICE_OBJS` only per the rule above)
8. If linker script is already patched, the patcher's stale-membership
   self-heal re-emits the late_pak block on the next build; for a main
   (`PRACTICE_OBJS`) file, manually add the `.o` to all four sections
   (`.text`, `.data`, `.rodata`, `.bss`)
9. If engine files must call into it or write its globals, add a Pak-gated
   shim / global to `src/practice/practice_shim.c` instead of referencing
   the Pak object directly (see `check_late_pak_segment`)
